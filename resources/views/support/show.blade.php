<x-app-layout>
    <title>Arewa Smart - Ticket #{{ $ticket->reference }}</title>
   
    <!-- ... (omitting CSS for brevity if not changing) ... -->
    <style>
        .message-bubble p {
                ...
                <div class="row align-items-center">
                    <div class="col-12 col-md-6 mb-2 mb-md-0">
                        <div class="d-flex align-items-center gap-2">
                            ...
                            <div>
                                <h4 class="fw-bold text-primary mb-0">Ticket #{{ $ticket->reference }}</h4>
    ...
    default => 'info'
                        ...
                        <div class="d-flex gap-2">
                            @if($ticket->status !== 'closed')
                                <button onclick="closeTicket('{{ $ticket->reference }}')"
                    ...
                    <div class="card shadow-lg border-0 chat-wrapper" style="border-radius: 20px; overflow: hidden;">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-end shadow-sm border-0">
                                        ...
                                        <li>
                                            <a class="dropdown-item text-danger" href="javascript:void(0)"
                                                onclick="closeTicket('{{ $ticket->reference }}')">
                        ...
                        <div class="card-body bg-chat overflow-auto p-3 p-md-4" id="messages-area" style="flex: 1;">
                            @foreach($messages as $message)
                                <div class="d-flex mb-4 {{ $message->role !== 'user' ? 'justify-content-start' : 'justify-content-end' }}"
                                    data-id="{{ $message->id }}">
                                    ...
                                    <div
                                        class="card border-0 message-bubble {{ $message->role !== 'user' ? 'message-admin' : 'message-user' }}">
                                        <div class="card-body p-3">
                                            <p class="mb-1 text-dark" style="line-height: 1.5;">{!! nl2br(e($message->content)) !!}</p>
                        ...
                        <div class="card-footer p-3 p-md-4 glass-footer" style="border-bottom-left-radius: 20px; border-bottom-right-radius: 20px;">
                            @if($ticket->status !== 'closed')
                                <form id="reply-form" action="{{ route('support.reply', $ticket->reference) }}"
                                    method="POST" enctype="multipart/form-data" class="w-100 no-loader">
                                    @csrf
        ...
        <script>
            ...
            function formatMarkdown(text) {
                if (!text) return '';
                // Bold (**text**)
                text = text.replace(/\*\*(.*?)\*\*/g, '<strong class="fw-bold">$1</strong>');
                // Bullet points (start of line - or *)
                text = text.replace(/^\s*[-*+]\s+(.*)$/gm, '<li class="ms-3">$1</li>');
                // Wrap lists in <ul>
                text = text.replace(/(<li.*<\/li>)/gs, '<ul class="mb-2 ps-0" style="list-style-type: disc;">$1</ul>');
                return text;
            }
            ...
            function appendMessage(msg) {
                ...
                const msgHtml = `
                    <div class="d-flex mb-4 ${isAdmin ? 'justify-content-start' : 'justify-content-end'}" data-id="${msg.id}" style="animation: fadeIn 0.3s ease-out;">
                        ${isAdmin ? `
                            <div class="me-2 d-none d-md-block">
                        ...
                        <div class="card border-0 message-bubble ${isAdmin ? 'message-admin' : 'message-user'}" style="background-color: ${isAdmin ? '#ffffff' : '#e3f2fd'}">
                            <div class="card-body p-3">
                                <p class="mb-1 text-dark" style="line-height: 1.5;">${formatMarkdown(msg.content)}</p>
                                        ...
                                        ${msg.attachment.match(/\.(jpg|jpeg|png|gif|webp)$/i) ? `
                                            <div class="mb-1">
                                                <a href="/storage/${msg.attachment}" target="_blank">
                                                    <img src="/storage/${msg.attachment}" alt="Attachment" class="img-fluid rounded border shadow-sm" style="max-width: 100%; max-height: 250px;">
                                                </a>
                                            </div>
                                        ` : `
                                            <a href="/storage/${msg.attachment}" target="_blank" class="d-flex align-items-center text-decoration-none">
                                                <i class="ti ti-file-text fs-15 me-2"></i>
                                                <span class="small fw-semibold">View Attachment</span>
                                            </a>
                                        `}
            ...
            function pollUpdates() {
                fetch(`{{ route('support.updates', $ticket->reference) }}?last_message_id=${lastMessageId}`, {
                    headers: {
                    ...
                    fetch(form.action, {
                        ...
                        .then(data => {
                            if (data.success) {
                                ...
                                if (data.message) {
                                    const tempBubble = document.querySelector(`[data-id="${tempMessage.id}"]`);
                                    if (tempBubble) tempBubble.setAttribute('data-id', data.message.id);
        ...
        <style>
            ...
            .focus-within-primary:focus-within { border-color: var(--bs-primary) !important; box-shadow: 0 0 0 0.25rem rgba(13, 110, 253, 0.25) !important; }
            #messages-area::-webkit-scrollbar { width: 6px; }
            #messages-area::-webkit-scrollbar-track { background: transparent; }
            #messages-area::-webkit-scrollbar-thumb { background: #dee2e6; border-radius: 10px; }
            #messages-area::-webkit-scrollbar-thumb:hover { background: #adb5bd; }
            .loader-dots span { width: 6px; height: 6px; margin: 0 1px; background-color: #fff; border-radius: 50%; display: inline-block; animation: bounce 0.6s infinite alternate; }
